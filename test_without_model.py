...
def test_browser():
    ...
    try:
        ...
        for i, elem in enumerate(elements[:5], 1):
            print(f"      {i}. {elem['tag']} - {elem.get('type', 'N/A')} - {elem.get('text', 'N/A')[:30]}")
        if len(elements) > 5:
        ...
        result = browser.execute_action("type", "input[name='my-text']", "Hello AI Agent!")
        ...
        result = browser.execute_action("type", "input[name='my-password']", "SecurePass123")
        if result.get("success"):
