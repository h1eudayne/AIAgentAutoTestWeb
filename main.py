...
from config.settings import (
    LLAMA_MODEL_PATH, LLAMA_N_CTX, LLAMA_N_GPU_LAYERS,
    BROWSER_HEADLESS, BROWSER_TIMEOUT, REPORTS_DIR
)
...
class AIWebTestAgent:
    def __init__(self, model_path: str = None, headless: bool = False):
        ...
        if not Path(model_path).exists():
            ...
            print(f"{Fore.YELLOW}💡 Download LLaMA 3 model and update LLAMA_MODEL_PATH{Style.RESET_ALL}")
            print(f"{Fore.YELLOW}   Example: https://huggingface.co/TheBloke/Llama-3-8B-GGUF{Style.RESET_ALL}")
            sys.exit(1)
...
def main():
    ...
    parser.add_argument(
        "--model",
        help="Đường dẫn đến LLaMA 3 model (.gguf)",
        default=None
    )
    parser.add_argument(
        "--headless",
