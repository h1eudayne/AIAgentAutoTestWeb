...
def demo_network_monitoring():
    ...
    try:
        ...
        print(f"{Fore.YELLOW}selenium-wire not installed. Skipping demo.{Style.RESET_ALL}")
...
def demo_self_healing():
    ...
    try:
        ...
        element, healed_selector = healer.find_element(
            browser.driver,
            "button:nth-of-type(99)",
            "button"
        )
        
        if element:
            print(f"   {Fore.GREEN}✓ Element found with healed selector!{Style.RESET_ALL}")
        else:
            print(f"   {Fore.YELLOW}⚠ No element found (page may not have buttons){Style.RESET_ALL}")
...
def demo_integrated():
    ...
    try:
        ...
        tracker.set_coverage_goals(
            ...
            features=["page_load", "visual_check"]
        )
