...
class FastWebTestAgent:
    def __init__(
        ...
        print(
            f"{Fore.CYAN}🤖 Initializing Fast Web Testing Agent (No LLM)...{Style.RESET_ALL}\n"
        )
        ...
        if enable_retry:
            print(
                f"{Fore.GREEN}✓ Retry logic enabled (max 3 attempts per action){Style.RESET_ALL}"
            )
        if enable_memory:
            print(
                f"{Fore.GREEN}✓ State memory enabled (learns from past tests){Style.RESET_ALL}"
            )
    ...
    def generate_tests_from_elements(self, elements: list) -> list:
        ...
        inputs = [
            ...
            for e in elements
            if e.get("tag") == "input"
            and e.get("type") in ["text", "email", "password", "search", None]
        ]
        ...
        for i, inp in enumerate(inputs[:3], 1):
            ...
            test_value = (
                "test@example.com" if inp.get("type") == "email" else "Test Input 123"
            )
            ...
            test_cases.append(
                {
                    ...
                    "priority": "low",
                    "steps": [{"action": "click", "selector": "a:first-of-type"}],
                }
            )
        ...
        try:
            ...
            if self.executor.enable_memory and self.executor.memory:
                ...
                if recommendations.get("test_stats", {}).get("total", 0) > 0:
                    ...
                    print(
                        f"  {Fore.CYAN}💾 Memory: Tested this page {stats['total']} times before (Pass rate: {stats['pass_rate']}){Style.RESET_ALL}"
                    )

            print(
                f"{Fore.GREEN}✓ Found {len(elements)} interactive elements{Style.RESET_ALL}"
            )
            ...
            if self.executor.enable_retry:
                ...
                if failed_actions:
                    print(
                        f"  {Fore.RED}Failed after retries: {len(failed_actions)}{Style.RESET_ALL}"
                    )
