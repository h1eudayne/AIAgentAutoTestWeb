...
class StateMemory:
    ...
    def remember_successful_selector(self, url: str, element_type: str, selector: str, context: Dict = None):
        """
        ...
        existing = None
        for i, s in enumerate(self.selector_memory[page_hash]["selectors"][element_type]):
        ...
        if existing is not None:
            ...
            self.selector_memory[page_hash]["selectors"][element_type][existing]["success_count"] += 1
            self.selector_memory[page_hash]["selectors"][element_type][existing]["last_used"] = datetime.now().isoformat()
        else:
            # Add new selector
            self.selector_memory[page_hash]["selectors"][element_type].append(selector_entry)
        
        # Update session
        self.current_session["successful_selectors"][selector] = self.current_session["successful_selectors"].get(selector, 0) + 1
        
        # Save to disk
        self._save_json(self.selector_memory_file, self.selector_memory)
    
    def remember_failed_selector(self, url: str, element_type: str, selector: str, error: str):
        """
        ...
        self.selector_memory[page_hash]["failed_selectors"][element_type].append(failed_entry)
    ...
    def get_best_selectors(self, url: str, element_type: str, limit: int = 5) -> List[str]:
        """
        ...
        sorted_selectors = sorted(selectors, key=lambda x: x["success_count"], reverse=True)
    ...
    def should_avoid_selector(self, url: str, element_type: str, selector: str) -> bool:
        ...
        failed_selectors = self.selector_memory[page_hash].get("failed_selectors", {}).get(element_type, [])
        
        # Check if selector failed recently (within last 10 attempts)
        recent_failures = [f for f in failed_selectors[-10:] if f["selector"] == selector]
    ...
    def get_test_statistics(self, url: Optional[str] = None) -> Dict:
        ...
        """
        if url:
            tests = [t for t in self.test_history if t.get("url") == url and t.get("type") != "session"]
        else:
    ...
    def learn_page_pattern(self, url: str, page_info: Dict):
        ...
        pattern = {
            "url": url,
            "element_counts": {
                "buttons": len([e for e in page_info.get("elements", []) if e.get("tag") == "button"]),
                "inputs": len([e for e in page_info.get("elements", []) if e.get("tag") == "input"]),
                "links": len([e for e in page_info.get("elements", []) if e.get("tag") == "a"])
            },
            "common_classes": self._extract_common_classes(page_info.get("elements", [])),
