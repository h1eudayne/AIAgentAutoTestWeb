...
class CoverageTracker:
    ...
    def __init__(self, output_dir: str = "reports/coverage"):
        ...
        self.actions_tested = defaultdict(lambda: defaultdict(int))  # page -> action -> count
    ...
    def set_coverage_goals(self, pages: List[str] = None, 
    ...
    def track_test_result(self, test_name: str, page: str, 
    ...
    def get_page_coverage(self) -> Dict:
        ...
        tested = len(self.pages_tested & self.coverage_goals["pages"]) if total_goals > 0 else len(self.pages_tested)
        ...
        return {
            ...
            "untested_pages": list(self.coverage_goals["pages"] - self.pages_tested) if total_goals > 0 else []
        }
    
    def get_element_coverage(self) -> Dict:
        """Get element coverage statistics"""
        total_elements = sum(len(elements) for elements in self.elements_tested.values())
        
        # Calculate success rate
        total_success = sum(data["success"] for data in self.element_success_rate.values())
        total_attempts = sum(data["success"] + data["fail"] for data in self.element_success_rate.values())
        success_rate = (total_success / total_attempts * 100) if total_attempts > 0 else 0
        
        return {
            "total_elements_tested": total_elements,
            "pages_with_elements": len(self.elements_tested),
            "avg_elements_per_page": round(total_elements / max(len(self.elements_tested), 1), 1),
    ...
    def get_action_coverage(self) -> Dict:
        ...
        return {
            ...
            "most_common_action": max(all_actions.items(), key=lambda x: x[1])[0] if all_actions else None
        }
    
    def get_feature_coverage(self) -> Dict:
        """Get feature coverage statistics"""
        total_goals = len(self.coverage_goals["features"])
        tested = len(self.features_tested & self.coverage_goals["features"]) if total_goals > 0 else len(self.features_tested)
        ...
        return {
            ...
            "coverage_percentage": round(coverage_pct, 1),
            "untested_features": list(self.coverage_goals["features"] - self.features_tested) if total_goals > 0 else []
        }
    ...
    def get_coverage_gaps(self) -> Dict:
        ...
        if self.coverage_goals["pages"]:
            gaps["untested_pages"] = list(self.coverage_goals["pages"] - self.pages_tested)
        
        # Untested features
        if self.coverage_goals["features"]:
            gaps["untested_features"] = list(self.coverage_goals["features"] - self.features_tested)
    ...
    def print_summary(self):
        ...
        if page_cov['goal_pages'] > 0:
            print(f"  Goal Pages: {page_cov['goal_pages_tested']}/{page_cov['goal_pages']} ({page_cov['coverage_percentage']}%)")
        ...
        if action_cov['actions_breakdown']:
            print(f"  Breakdown:")
            for action, count in sorted(action_cov['actions_breakdown'].items(), key=lambda x: x[1], reverse=True):
                print(f"    • {action}: {count}")
        
        # Feature coverage
        if feature_cov['goal_features'] > 0:
            print(f"\n{Fore.YELLOW}Feature Coverage:{Style.RESET_ALL}")
            print(f"  Features Tested: {feature_cov['goal_features_tested']}/{feature_cov['goal_features']} ({feature_cov['coverage_percentage']}%)")
