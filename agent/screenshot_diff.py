...
class ScreenshotDiff:
    ...
    def compare(self, name: str, threshold: Optional[float] = None) -> Dict:
        ...
        try:
            ...
            mismatch_pixels = pixelmatch(
                img_baseline, img_current, img_diff, threshold=threshold, includeAA=True
            )
    ...
    def _print_comparison_result(self, result: Dict):
        ...
        if status == "identical":
            ...
            print(
                f"  {Fore.YELLOW}⚠ {name}: Minor diff ({mismatch_pct}%){Style.RESET_ALL}"
            )
        elif status == "moderate_diff":
            print(
                f"  {Fore.YELLOW}⚠ {name}: Moderate diff ({mismatch_pct}%){Style.RESET_ALL}"
            )
        elif status == "major_diff":
            print(
                f"  {Fore.RED}✗ {name}: Major diff ({mismatch_pct}%){Style.RESET_ALL}"
            )
        elif status == "no_baseline":
            print(f"  {Fore.CYAN}ℹ {name}: No baseline (creating){Style.RESET_ALL}")
        elif status == "error":
            print(
                f"  {Fore.RED}✗ {name}: Error - {result.get('message')}{Style.RESET_ALL}"
            )
    ...
    def print_summary(self):
        ...
        print(
            f"{Fore.YELLOW}⚠ Moderate Diff: {summary['moderate_diff']}{Style.RESET_ALL}"
        )
        print(f"{Fore.RED}✗ Major Diff: {summary['major_diff']}{Style.RESET_ALL}")

        if summary.get("no_baseline", 0) > 0:
            print(
                f"{Fore.CYAN}ℹ No Baseline: {summary['no_baseline']}{Style.RESET_ALL}"
            )
        ...
        # Show major diffs
        major_diffs = [
            r for r in self.comparison_results if r["status"] == "major_diff"
        ]
        if major_diffs:
