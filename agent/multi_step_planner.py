...
class MultiStepPlanner:
    ...
    def create_plan_from_template(
        self, template_name: str, plan_id: str
    ) -> Optional[TestPlan]:
        ...
        plan = TestPlan(
            id=plan_id, name=template["name"], description=template["description"]
        )
    ...
    def create_custom_plan(
        self, plan_id: str, name: str, description: str, steps_data: List[Dict]
    ) -> TestPlan:
        """Tạo custom plan"""
        plan = TestPlan(id=plan_id, name=name, description=description)
    ...
    def visualize_plan(self, plan: TestPlan) -> str:
        ...
        lines.append(
            f"Progress: {plan.get_progress()['progress']} ({plan.get_progress()['percentage']:.1f}%)"
        )
        lines.append(f"\n{'Steps:'}")
        lines.append(f"{'-'*60}")
