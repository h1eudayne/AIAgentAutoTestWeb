...
class TestPlanner:
    ...
    def analyze_page(self, page_info: Dict) -> Dict:
        ...
        elements = str(page_info.get("interactive_elements", []))[
            :800
        ]  # Reduced from 1000
    ...
    def generate_test_cases(
        self, page_analysis: Dict, elements: List[Dict]
    ) -> List[Dict]:
    ...
    def _generate_basic_tests(self, elements: List[Dict]) -> List[Dict]:
        ...
        # Find buttons
        buttons = [
            e for e in elements if e.get("tag") == "button" or e.get("type") == "submit"
        ]
        if buttons:
        ...
        inputs = [
            ...
            for e in elements
            if e.get("tag") == "input"
            and e.get("type") in ["text", "email", "password"]
        ]
        if inputs:
        ...
        return (
            ...
            if test_cases
            else [
                {
                    ...
                    "priority": "high",
                    "steps": [
                        {"action": "wait", "value": "2", "expected": "page loaded"}
                    ],
                }
            ]
        )
