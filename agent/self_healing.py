...
class SelfHealingSelector:
    ...
    def find_element(self, driver, selector: str, element_type: str = "button") -> Tuple[Optional[object], Optional[str]]:
        """
    ...
    def _heal_by_id(self, driver, selector: str, element_type: str) -> Optional[Tuple]:
        ...
        try:
            ...
            if '#' in selector:
                element_id = selector.split('#')[1].split('.')[0].split('[')[0].split(':')[0]
    ...
    def _heal_by_text(self, driver, selector: str, element_type: str) -> Optional[Tuple]:
    ...
    def _heal_by_attributes(self, driver, selector: str, element_type: str) -> Optional[Tuple]:
        """Strategy 3: Find by common attributes"""
        try:
            # Common attributes to check
            attributes = ['data-testid', 'data-test', 'name', 'aria-label', 'title', 'type']
    ...
    def _heal_by_position(self, driver, selector: str, element_type: str) -> Optional[Tuple]:
    ...
    def _heal_by_similarity(self, driver, selector: str, element_type: str) -> Optional[Tuple]:
    ...
    def _heal_by_parent_context(self, driver, selector: str, element_type: str) -> Optional[Tuple]:
        """Strategy 6: Find by parent context"""
        try:
            # Try to find parent container first
            parent_selectors = ['form', 'div.container', 'div.content', 'main', 'section']
    ...
    def get_healing_stats(self) -> Dict:
        ...
        return {
            ...
            "most_effective_strategy": max(strategies.items(), key=lambda x: x[1])[0] if strategies else None
        }
    ...
    def print_healing_summary(self):
        ...
        if stats['strategies_used']:
            ...
            for strategy, count in sorted(stats['strategies_used'].items(), key=lambda x: x[1], reverse=True):
                strategy_name = strategy.replace('_heal_by_', '').replace('_', ' ').title()
                print(f"  • {strategy_name}: {count}")
        
        if stats['most_effective_strategy']:
            strategy_name = stats['most_effective_strategy'].replace('_heal_by_', '').replace('_', ' ').title()
        ...
        if self.healing_history:
            ...
            for record in self.healing_history[-5:]:
                print(f"  {record['original_selector'][:40]} → {record['healed_selector'][:40]}")
