...
class RetryHandler:
    ...
    def execute_with_retry(
        ...
        while attempt < self.max_retries:
            ...
            try:
                print(
                    f"  {Fore.CYAN}[Attempt {attempt}/{self.max_retries}]{Style.RESET_ALL} {action_name}"
                )
                ...
                if result.get("success"):
                    if attempt > 1:
                        print(
                            f"  {Fore.GREEN}✓ Success after {attempt} attempts{Style.RESET_ALL}"
                        )
                ...
                print(
                    f"  {Fore.YELLOW}⚠ Attempt {attempt} failed: {last_error}{Style.RESET_ALL}"
                )
                ...
                if attempt < self.max_retries:
                    ...
                    print(
                        f"  {Fore.CYAN}💡 Retry strategy: {strategy['description']}{Style.RESET_ALL}"
                    )
                ...
                last_error = str(e)
                print(
                    f"  {Fore.RED}✗ Exception in attempt {attempt}: {last_error}{Style.RESET_ALL}"
                )
...
class SmartSelector:
    ...
    def generate_alternatives(
        original_selector: str, element_info: Dict = None
    ) -> List[str]:
        """
        ...
        if element_info:
            ...
            if elem_text:
                # XPath by text
                alternatives.append(
                    f"//{elem_tag}[contains(text(), '{elem_text[:20]}')]"
                )
...
class RetryableAction:
    ...
    def click_with_retry(self, selector: str, element_info: Dict = None) -> Dict:
        ...
        result = self.retry_handler.execute_with_retry(
            click_action, f"Click: {selector_display}"
        )

        # If failed, try alternative selectors
        if not result.get("success") and element_info:
            alternatives = self.smart_selector.generate_alternatives(
                selector, element_info
            )

            for alt_selector in alternatives[1:]:  # Skip first (original)
                print(
                    f"  {Fore.CYAN}🔄 Trying alternative selector: {alt_selector[:50]}{Style.RESET_ALL}"
                )
    ...
    def type_with_retry(
        self, selector: str, value: str, element_info: Dict = None
    ) -> Dict:
        ...
        result = self.retry_handler.execute_with_retry(
            type_action, f"Type: {selector[:50]}"
        )
