...
class ChatbotTester:
    ...
    def test_question(self, test_case: dict, index: int, total: int):
        ...
        print(f"{Fore.CYAN}[Test {index}/{total}] Category: {test_case['category']}{Style.RESET_ALL}")
    ...
    def generate_report(self, url: str):
        ...
        avg_accuracy = sum(r.get("keyword_check", {}).get("accuracy", 0) 
                          for r in self.test_results) / total if total > 0 else 0
        ...
        for r in self.test_results:
            ...
            categories[cat]["accuracy"].append(r.get("keyword_check", {}).get("accuracy", 0))
