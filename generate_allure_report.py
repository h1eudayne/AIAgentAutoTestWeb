...
def generate_report():
    ...
    if not results_dir.exists() or not list(results_dir.glob("*")):
        ...
        print(
            f"{Fore.YELLOW}Run tests first with: pytest --alluredir=allure-results{Style.RESET_ALL}"
        )
        return False
...
def serve_report():
    ...
    if not report_dir.exists():
        ...
        print(
            f"{Fore.YELLOW}Generate report first with: allure generate{Style.RESET_ALL}"
        )
        return False
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate and serve Allure reports")
    parser.add_argument(
        "--test", action="store_true", help="Run tests before generating report"
    )
    parser.add_argument(
        "--generate", action="store_true", help="Generate report only (no serve)"
    )
    parser.add_argument("--serve", action="store_true", help="Serve existing report")
